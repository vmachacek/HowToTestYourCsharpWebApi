...
namespace HowToTestYourCsharpWebApi.Api.ExternalApi
{
    ...
    public class Rates
    {
        ...
        {
            return Convert.ToDecimal(GetType()?.GetProperty(currency).GetValue(this));
        }

        public Money ConvertToEur(Money objPrice)
        {
            if (objPrice.Currency == "EUR")
                return objPrice;

            var value = GetValue(objPrice.Currency.ToUpper());

            return new Money("EUR", objPrice.Amount / value);
        }
    }
}